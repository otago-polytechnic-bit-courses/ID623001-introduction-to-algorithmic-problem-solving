...
- Next comes the **true** and **false** parts of the if statement: `0 : (Random.value < .5 ? -1 : 1);` - The `:` is like the `else` in this scenario... *if* the condition is **true** then assign `a = 0`... *else* assign `a = (Random.value < .5 ? -1 : 1)`.
